                ...
                <tbody>
                    @foreach($pemeriksaan as $item)
                        <tr>
                            ...
                            <td>{{ $item->evaluasi }}</td>
                            <td>
                                <button type="button" class="btn btn-primary" onclick="copyToForm({{ json_encode($item) }})">Copy</button>
                            </td>
                            <td>
                                <form action="{{ route('pemeriksaan.destroy', ['no_rawat' => $item->no_rawat, 'jam_rawat' => $item->jam_rawat]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="container mt-5">
        ...
        <form action="{{ route('perawatan.soap') }}" method="POST">
            ...
            <div class="form-group mb-2">
                ...
                <input type="text" class="form-control" id="pasien" value="{{ $pasien->pasien->nm_pasien }}"
                    readonly>
            </div>
